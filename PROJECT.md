...
- ~~**No live lineup data**~~: ✅ **RESOLVED June 10 late evening** — `bin/populate_wc_lineups.py` is the auto-population job (iterates KXWCGAME markets, resolves FotMob matchIds via `/api/data/matches?date=...`, scrapes lineups 60-90 min pre-kickoff, writes to `data/cache/worldcup/lineups.json` + `fotmob_ids.json`). Two wiring layers: (1) **hourly cron** via `scripts/install-cron.sh` (schedule: `0 7-23 11-19 6,7 *` = 7am-11pm, June 11 - July 19, WC 2026 window) running `bin/populate_wc_lineups.py`; (2) **pre-step hook** in `morning_scan.py` section 2.5 (subprocess with `--window 90 --lookahead 4`, swallows errors so morning_scan is never blocked). **Installed + smoke-tested late-night June 10** — `bash scripts/install-cron.sh` registered both crons, `bash scripts/install-cron.sh --status` shows both ✅ INSTALLED, `python -m src.scripts.morning_scan --paper` ran the full pipeline in ~3 min, section 2.5 pre-step fired and section 3 picked up 216 KXWCGAME markets / 325 qualifying paper bets. Dry-run verified: 216 KXWCGAME markets → 72 unique matches → 2 in 24h horizon → 1 FotMob matchId resolved (Mexico vs RSA, 4667751), 1 needs-resolve (Korea vs Czechia — FotMob hasn't listed yet, graceful skip). **26/26 unit tests pass** for ticker parsing + time-window filtering. Live e2e against BOL/ALG (~01:00 UTC June 11) still pending — test's date filter uses PDT (20260610) instead of UTC (20260611) where the WC Grp. A matches actually live; conftest fix landed so `--runlive` flag now registers.
...
| **🐛 `resolve_paper_trades.py` result-field type bug** | **Lifetime NBA win rate is 0% (0/889) and MLB is 0% (0/559) — but it's a code bug, not bad luck.** Kalshi's `result` field is a STRING ("yes"/"no"); the script does `_safe_float(result)` which returns 0.0 for any string. `won = (0.0 >= 0.5)` is always False for YES-side bets → ALL YES-side resolutions marked LOST. Cumulative impact: **−$289.81 across NBA lifetime, −$XXX across MLB lifetime** (also broken but not yet totaled). Traced trade #7108 to confirm. | **Fix Branch 1** in `src/scripts/resolve_paper_trades.py`: replace `_safe_float(result)` with direct string check `(str(result).lower()=='yes') if side=='yes' else (str(result).lower()=='no')`. **Fix Branch 3** (bid=ask=0): can't tell which side won → `skipped += 1` instead of forcing LOST. **Then reset DB**: `UPDATE trades SET status='pending', resolved_price=NULL, pnl=0.0 WHERE status='lost' AND sport IN ('nba','mlb')` and re-run. **Add unit test** `tests/test_resolve_paper_trades.py` that mocks KalshiClient.list_markets with `result='yes'`, `result='no'`, `result=None` and asserts the script maps correctly. |
...
- [ ] **🐛 CRITICAL: Fix `resolve_paper_trades.py` result-field type bug** — see Known Issues & Gaps. Lifetime NBA 0/889 won, MLB 0/559 won. Patch Branch 1 (string result) + Branch 3 (bid=ask=0), reset LOST trades, re-run, add unit test.
