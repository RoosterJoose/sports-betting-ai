#!/usr/bin/env python3
"""Fit Beta Calibration for NBA player stat models.

Loads the trained XGBoost NBA models, runs a temporal train/test split,
computes raw NB/Poisson probabilities for various lines, then fits
BetaCalibrator to correct systematic bias.

Usage:
    python scripts/fit_nba_beta_cal.py
"""
...
def load_features():
    """Load NBA data and build features using NBAFeatureEngineer."""
    ...
    # Compute combo stat columns for PRA, PA, PR, RA
    if all(c in featured.columns for c in ["pts", "reb", "ast"]):
        ...
        print(f"  Computed combo stats: pra, pa, pr, ra", flush=True)
...
def fit_calibration(stat_name: str, model_display: str, featured: pd.DataFrame):
    ...
    # Get raw stat column name — NBA raw data columns are lowercase stat names
    # e.g., PTS -> pts, FG3M -> fg3m, FTM -> ftm
    raw_col = stat_name.lower()
    if raw_col not in featured.columns:
        print(f"  {stat_name}: column '{raw_col}' not found in features")
        # Try to find it from the raw game data
        cols_found = [c for c in featured.columns if c in [
            "pts", "reb", "ast", "stl", "blk", "tov", "fg3m", "fg3a",
            "fgm", "fga", "ftm", "fta", "min", "oreb", "dreb", "pf"
        ]]
        print(f"    Available raw stat columns: {cols_found}")
        return
    ...
    print(f"  {stat_name:4s}: {len(X_test)} test rows, σ={std:.3f}", flush=True)
    ...
    for line_val in range(min_line, max_line + 1):
        # Compute P(stat >= line) for each test row
        p_model = np.array([
    ...
    # Filter out trivial predictions (p near 0 or 1)
    ...
    # Also compute calibration by bins
    bins = np.linspace(0, 1, 11)
    bin_centers = (bins[:-1] + bins[1:]) / 2
    bin_map = {i: [] for i in range(10)}
    for i in range(len(valid_arr := raw_arr[valid])):
        if valid_arr[i] >= 0.99:
            bin_idx = 9
        else:
            bin_idx = min(9, max(0, int(valid_arr[i] * 10)))
    ...
    if raw_cal and cal_cal:
        avg_raw_cal_err = float(np.mean(raw_cal))
        avg_cal_cal_err = float(np.mean(cal_cal))
        print(f"    Cal error: {avg_raw_cal_err:.4f} → {avg_cal_cal_err:.4f} "
              f"({'✅ improved' if avg_cal_cal_err < avg_raw_cal_err else '❌ worse'})")
    ...
    # Also save calibration diagnostics
    diag = {
        "stat": stat_name,
...
def main():
    print("=" * 65)
    print("  NBA BETA CALIBRATION FITTER")
    print("=" * 65)
    ...
    print(f"\n{'=' * 65}")
    print("  Done. Calibration files saved to models/nba/")
    for f in sorted(MODEL_DIR.glob("*beta_cal*")):
        print(f"    {f.name}")

    # Summary
    print(f"\n  {'Stat':6s} {'a':>8s} {'b':>8s} {'c':>8s} {'N_cal':>6s} {'Bias':>8s}")
    print(f"  {'-'*6} {'-'*8} {'-'*8} {'-'*8} {'-'*6} {'-'*8}")
    for stat_name, _ in STAT_TYPES:
        diag_path = MODEL_DIR / f"{stat_name.lower()}_calibration_diag.json"
        if diag_path.exists():
            with open(diag_path) as f:
                d = json.load(f)
            print(f"  {stat_name:6s} {d['a']:>8.3f} {d['b']:>8.3f} {d['c']:>8.3f} "
